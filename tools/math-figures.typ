...
#let right_triangle(
  ...
  #cetz.canvas(length: default_canvas_length, {
    ...
    if height_label != none {
      content((-0.35, 1.5), [#height_label])
    }

    if hypotenuse_label != none {
      content((2.0, 1.7), [#hypotenuse_label])
    }
  })
]
...
// --------------------------------------------------
#let cone(height_value, radius_value, unit: "") = {
  let rx = 2.0
  let ry = 0.45
  let H = 4.0

  let height_text =
    if unit == "" { [#height_value] } else { [#height_value #unit] }

  let radius_text =
    if unit == "" { [#radius_value] } else { [#radius_value #unit] }
